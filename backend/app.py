...
def get_tasks():
    """Retrieves all tasks."""
    tasks = query_db('SELECT id, title, description, due_datetime FROM tasks')
...
def add_task():
    ...
    task_id = execute_db(
        'INSERT INTO tasks (title, description, due_datetime) VALUES (?, ?, ?)',
        (title, description, due_datetime)
    )
    task = query_db('SELECT id, title, description, due_datetime FROM tasks WHERE id = ?', [task_id], one=True)
